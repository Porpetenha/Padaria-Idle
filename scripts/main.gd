...
func set_label(): #OK
	money_label.text = Global.format_number(Global.money)
	bread_production.text = "Produção: \n" + Global.format_number(bread_click) + " /click \n" + Global.format_number(bread_idle) + " /sec" 
	
func _on_click_forno_pressed() -> void: #OK
	Global.money += bread_click #Qando clicar no botão do forno vai produzir dinherio por click
	
func _on_idle_timer_timeout() -> void:
	Global.money += bread_idle #Função para a geração idle de dinheiro
...
func production_calc():
	...
	for pao in Global.paes_data:
		if pao.upgrade_type == UpgradeData.upgrade_type_list.BREAD:
			bread_idle += pao.bonus_value * pao.level 
